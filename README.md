# Cybersecurity Portfolio - USIU-Africa Applied Computer Technology

This repository is a beginner-to-intermediate cybersecurity portfolio for a USIU-Africa Applied Computer Technology student specializing in cybersecurity. It combines safe hands-on Python projects with structured security documentation.

## Table of Contents
- [About](#about)
- [Skills and Technologies](#skills-and-technologies)
- [Projects](#projects)
- [Current Learning Goals](#current-learning-goals)
- [Portfolio Roadmap](#portfolio-roadmap)
- [Ethical Use and Legal Notice](#ethical-use-and-legal-notice)
- [Contact](#contact)

## About
I am building practical cybersecurity skills in defensive engineering, log analysis, secure coding, and digital investigation workflows. Every project here is designed for authorized lab learning and professional growth.

## Skills and Technologies
- Python
- Bash
- Linux
- Networking fundamentals
- Git and GitHub
- Wireshark
- Nmap (safe lab usage)
- Web security fundamentals
- SOC monitoring fundamentals
- Digital forensics fundamentals

## Projects
- [Python Network Scanner](projects/python-network-scanner/)
- [Linux Log Analyzer](projects/linux-log-analyzer/)
- [File Integrity Monitor](projects/file-integrity-monitor/)
- [Secure Login Demo](projects/secure-login-demo/)
- [Wireshark Network Analysis](projects/wireshark-network-analysis/)
- [SOC Home Lab](projects/soc-home-lab/)
- [Digital Forensics Case Study](projects/digital-forensics-case-study/)
- [Security Awareness Toolkit](projects/security-awareness-toolkit/)
- [Linux Security Notes](projects/linux-security-notes/)

## Current Learning Goals
- Improve Python security tooling quality with tests and error handling.
- Strengthen Linux and network visibility workflows.
- Build SOC-style detection and triage thinking.
- Practice secure web authentication patterns.
- Improve investigation documentation and evidence handling discipline.

## Portfolio Roadmap
- **Phase 1:** Build and test Python CLI projects.
- **Phase 2:** Expand SOC, Wireshark, and forensics documentation.
- **Phase 3:** Improve automation, testing depth, and reporting quality.
- **Phase 4:** Add more realistic but still sanitized defensive scenarios.

## Ethical Use and Legal Notice
All content is for educational and authorized testing purposes only. Do not scan, test, or access systems without explicit permission. The repository intentionally avoids offensive exploitation workflows and real credentials.

## Contact
- LinkedIn: `REPLACE_WITH_LINKEDIN_URL`
- Email: `REPLACE_WITH_PROFESSIONAL_EMAIL`
